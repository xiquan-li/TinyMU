...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.config.enable_tscam:
            ...
            fine_grained_latent_out_worepeat = torch.mean(x, dim=2).transpose(-2, -1)
            fine_grained_latent_output = interpolate(fine_grained_latent_out_worepeat.contiguous(),
            ...
            if self.config.loss_type == "clip_ce":
                output_dict = {
                    ...
                    'clipwise_output': x,
                    'latent_output': latent_output,  # clip-level latent
                    "fine_grained_latent_output": fine_grained_latent_output,
                    "fine_grained_latent_out_worepeat": fine_grained_latent_out_worepeat
                }
            else:
                output_dict = {
                    'framewise_output': fpx, # already sigmoided
...
class HTSATWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.c2l_first == False: 
            ...
            clipwise_output = out_dict["clipwise_output"].unsqueeze(1)
            clipwise_output = self.c2l(clipwise_output)
            if self.wo_repeat: 
                frame_latent = out_dict["fine_grained_latent_out_worepeat"]
            else: 
                frame_latent = out_dict["fine_grained_latent_output"]
            out_dict["embedding"] = torch.concat((clipwise_output, frame_latent), axis=1)
